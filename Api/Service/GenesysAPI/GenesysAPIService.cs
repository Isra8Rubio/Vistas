...
namespace Api.Service.GenesysAPI
{
    public class GenesysAPIService
    {
        ...
        public readonly ILogger<GenesysAPIService> _logger;
        public GenesysAPIService(ILogger<GenesysAPIService> logger)
        {
            _logger = logger;
        }
        ...
        protected async Task<T> ExecuteWithRetry<T>(Func<Task<T>> action, string actionName)
        {
            ...
            while (true) // Bucle infinito para seguir intentando hasta que tenga éxito o se produzca un error no manejado.
            {
                ...
                {
                    _logger.LogError(ex, "{actionName} ExecuteWithRetry error", actionName); // Registra el error.
                    throw; // Lanza la excepción para que sea manejada por el llamador.
                }
                _logger.LogDebug("{action} Entro {cont}", actionName, cont++);
        ...
        protected void HandleNullResult(int retryCount, string method)
        {
            ...
            {
                _logger.LogError("Max retry attempts exceeded. Aborting.");
            ...
            _logger.LogWarning("[{method}] Rate limit exceeded. Retrying in {RetryAfter} seconds...", method, retryAfterSeconds);
